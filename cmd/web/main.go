...
func main() {
	...
	// This read in the command-line flag value adn assigns it to the addr variable.
	...
	// to the standard out stream and uses the default settings.
	logger := slog.New(slog.NewJSONHandler(os.Stdout, &slog.HandlerOptions{
		// AddSource: true,
		Level: slog.LevelDebug,
	}))

	// To kepp the main() function tidy I've put the code for creating a connection pool
	// into the separate openDB() function below. We pass openDB() the DSN from the
	// command-line flag.
	db, err := openDB(*dsn)
	if err != nil {
	...
	// We also defer a call to db.Close(), so that the connection pool is closed before
